...
# It is designed to merge two polygon datasets together (a base map and a set of new features),
# while avoiding slivers due to boundary mismatch. It identifies polygons in the base map
# that need to be split to match the boundaries of the new features, ignoring minor differences
# in the boundaries due to inaccurate mapping (which would generate slivers). The output is faithful
# to the base map boundaries as far as possible, but there is a trade-off between eliminating
# slivers and maintaining this integrity.
#
...
# depending on the datasets, and should be determined through inspecting the outputs and adjusting
...
#
# Change Log:
# V3: sort TI tables by size before joining, to deal with one-to-many issues. We can then
# use Interpretation to join attributes.
# V4: major rewrite to just apply one tabulate intersection step, for the base map
# polygons. The previous two-step system does not work well, especially where some of the new
# feature polygons are much larger than the base map polygons (e.g. for AONBs).
# V5: making more generic so it can handle different merge types (OSMM and HLU, then designations)
# Also removed Interpretation field, as it is not needed - we only need Relationship field.
# ------------------------------------------------------------------------------------------
