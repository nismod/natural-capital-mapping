...
# Output location
out_gdb = r"D:\cenv0389\OxCamArc\Comparison.gdb"
...
comp_fields = ["Interpreted_habitat", "ALC_GRADE", "NatureDesig", "CultureDesig", "EdDesig", "AccessType"]
...
comp_fields_new = ["Interpreted_habitat_1", "ALC_GRADE_12", "NatureDesig_1", "CultureDesig_1", "EdDesig_1", "AccessType_1"]
comp_fields_replace_null_with = ["", "Non Agricultural", 0, 0, 0, 0, ""]
NumCompFields = len(comp_fields)

# Which stages do we want to run?
split_compare_fc = False
replace_nulls = True
id_non_matching = True
...
if split_compare_fc:
    ...
    for LAD in LADs:
        ...
        split_fc = os.path.join(in_compare_gdb, LAD_name + in_compare_suffix)
...
for fc in in_fcs:
    ...
    if id_non_matching:
        in_fc = os.path.join(main_gdb, fc)
        in_compare_fc = os.path.join(in_compare_gdb, LAD_name + in_compare_suffix)

        compare_fc = LAD_name + out_suffix
        print("Running identity on " + in_fc)
        arcpy.Identity_analysis(in_fc, in_compare_fc, compare_fc, "NO_FID")
        ...
        if replace_nulls:
            ...
            for comp_field in comp_fields:
                ...
                if isinstance(comp_fields_replace_null_with[i-1], str):
                    ...
                    expression = comp_fields_replace_null_with[i-1]
                MyFunctions.select_and_copy(compare_fc, comp_field, comp_field + " IS NULL", expression)
            i = 0
            for comp_field_new in comp_fields_new:
                ...
                if isinstance(comp_fields_replace_null_with[i-1], str):
                    ...
                    expression = comp_fields_replace_null_with[i-1]
                MyFunctions.select_and_copy(compare_fc, comp_field_new, comp_field_new + " IS NULL", expression)
        ...
        for comp_field_short in comp_fields_short:
            ...
            MyFunctions.select_and_copy(compare_fc, comp_field_short + "_comp", expression, 1)
        ...
        MyFunctions.check_and_add_field(compare_fc, "CompareAll", "SHORT", "")
        MyFunctions.select_and_copy(compare_fc, "CompareAll", comp_expression, 1)

        # Select and export non-matching polygons
        print("Exporting non-matching polygons")
        arcpy.MakeFeatureLayer_management(compare_fc, "Non_matching_lyr", where_clause="CompareAll IS NULL OR CompareAll = 0")
        arcpy.CopyFeatures_management("Non_matching_lyr", compare_fc + "_non_matching")
        print("Finished comparison for " + LAD_name + " at " + time.ctime())

    # Identify common reasons for non-matching polygons
    non_match_fc = compare_fc + "_non_matching"
    ...
    # Special case for linear habitats
    expression = "HabNmPLUS_1 ='Linear habitats'"
    MyFunctions.select_and_copy(non_match_fc, "SimpleReason", expression, "'Undefined linear habitat mismatch'")
    ...
    # MyFunctions.select_and_copy(non_match_fc, "DetailedReason", expression, "'False ALC mismatch'")
    # Mis-matched Access
    expression = "DetailedReason IS NULL AND AccessType <> AccessType_1"
