...
if region == "Oxon" and method == "HLU":
    ...
    gdbs = [os.path.join(folder, "Merge_OSMM_HLU.gdb")]   # No longer exists...replace with LADs or new gdb (need to set up)
    LAD_table = os.path.join(folder, "Data.gdb", "Oxon_LADs")
    in_file_name = "OSMM_HLU"
    Hab_field = "PHASE1HABI"
    BAP_field = "BAP_HABITA"
...
def Interpret_hab(HLU_hab, OSMM_hab, OSMM_Make, OSMM_area, HLU_area, undefined_or_original):
 ...
 """
        ...
        if interpret_BAP:
            ...
            MyFunctions.check_and_add_field(in_file, "BAP_Interpretation", "TEXT", 100)
...
def interpretBAP(Simple, BAP):
    if BAP:
        ...
        return Simple
"""
            arcpy.CalculateField_management(in_file, "BAP_Interpretation",
